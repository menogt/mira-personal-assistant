...
export default async function DashboardPage() {
  ...
  const [todayTasks, upcomingDeadlines] = await Promise.all([
    getTodayTasks(user.id, profile.timezone),
    getUpcomingDeadlines(user.id, profile.timezone),
  ]);
  ...
  return <div className="space-y-8">
    ...
    <section className="grid gap-5 lg:grid-cols-3">
      ...
      <article className="min-h-48 rounded-2xl border border-zinc-800 bg-zinc-950 p-6"><CalendarDays className="h-5 w-5 text-zinc-500" /><h2 className="mt-5 text-lg font-semibold text-zinc-100">Today’s university schedule</h2><p className="mt-3 text-sm leading-6 text-zinc-500">Module schedule details are available in the University Hub.</p><p className="mt-6 text-xs font-medium uppercase tracking-[0.16em] text-zinc-600">University Hub</p></article>
    </section>
    <section className="grid gap-5 lg:grid-cols-2"><article className="rounded-2xl border border-zinc-800 bg-zinc-950 p-6"><Flag className="h-5 w-5 text-zinc-500" /><h2 className="mt-5 text-lg font-semibold text-zinc-100">Upcoming deadlines</h2>{upcomingDeadlines.length?<ul className="mt-4 space-y-3">{upcomingDeadlines.slice(0,6).map((d,i)=><li key={`${d.title}-${i}`} className="flex items-start justify-between gap-3 text-sm"><div><p className="font-medium text-zinc-200">{d.title}</p><p className="mt-1 text-xs text-zinc-500">{d.source} · {d.kind}</p></div><span className="shrink-0 text-xs text-emerald-300">{d.date}</span></li>)}</ul>:<p className="mt-3 text-sm leading-6 text-zinc-500">No project or university deadlines in the next seven days. Suspiciously peaceful.</p>}</article><article className="rounded-2xl border border-dashed border-zinc-800 bg-zinc-950/60 p-6"><p className="text-sm font-medium uppercase tracking-[0.18em] text-zinc-500">Recent progress</p><p className="mt-3 text-sm leading-6 text-zinc-400">Completion history and daily check-ins will appear here once those workflows are enabled.</p></article></section>
  </div>;
}
